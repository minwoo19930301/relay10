...
const summary = '현재 상태: 코드와 공개 저장소는 준비됐지만 v0.1.0 릴리스는 보류 중이다. Relay10은 Codex 작업 단계를 연결하는 작은 명령줄 도구다. 2026-07-13 현재 73개 자동 테스트, 문법 검사, 실제 모델 탐색, 모델 배정 미리보기, 패키지 검사를 통과했다. 자동 구조 검사의 10/10은 모델을 쓰지 않는 형식 검사 점수다. 최신 실제 저비용 모델 독자 검수는 5/10으로 실패했다. 자동 구조 점수와 실제 독자 점수가 같은 종류처럼 보여 혼란스럽다는 지적을 받아, 이 수정본은 둘을 명확히 분리했다. 최종 실제 독자 검수가 9/10 이상이고 치명적 문제 0개가 될 때만 릴리스한다. 남은 위험은 초기 규칙 기반 모델 배정의 실전 정확도, 같은 모델 계열 판단의 상관성, 비교 프로젝트의 빠른 변경이다. 용어: OMO(Oh My OpenAgent), OMP(Oh My Pi), OMC(Oh My ClaudeCode), OMX(Oh My Codex), GJC(Gajae-Code), CLI(명령줄 인터페이스), HTML(웹 문서 형식), URL(웹 주소), MIT(오픈소스 라이선스), LICENSE(라이선스 파일), RPC(원격 프로시저 호출), DSL(도메인 전용 언어), Reader-10(열 가지 독자 역할 검수), 하네스(작업 단계를 연결하는 실행 도구), 라우팅(단계별 모델 배정), economy(가벼운 역할), balanced(중간 역할), frontier(고성능 역할), low(낮은 추론 노력).';
...
const evidence = [
  ...
  { title: '최신 실제 Reader-10 실패 기록', url: `${repository}/blob/main/docs/launch-reader-live.json`, note: 'gpt-5.6-luna/low 10회 중 5회 통과. 구조 10/10과 실제 점수의 구분이 불명확하다는 지적을 이번 수정에 반영했다.' },
...
const verificationChecks = [
  ...
  { name: '최신 실제 저비용 독자 검수', passed: false, detail: 'gpt-5.6-luna/low 10회 중 5회 통과. 자동 구조 10/10과 실제 독자 점수를 같은 종류로 읽게 만든 표현 때문에 실패했다. 이 수정본은 자동 구조와 실제 모델 검수를 분리해 표시한다.' },
];
...
const stages = [
  ...
  },
  {
    ...
    output: '사실: Codex 위에 deep-interview, ralplan, ultragoal, /goal, worktree와 tmux team runtime을 얹는다. 장점: 목표·근거·격리 작업공간을 오래 유지하는 운영 흐름. 단점: setup scope, hooks, tmux, worktree, .omx 상태를 이해해야 한다. README는 MIT를 말하지만 조사 시점 root 표준 LICENSE 파일은 비어 있어 재사용 전 재확인이 필요하다.',
  },
  {
    title: 'GJC(Gajae-Code)', status: 'pass', profile: '국내 비교', model: '공식 저장소',
    summary: '장점은 좁은 공개 workflow, 단점은 내부 구현과 실험 기능이 여전히 크다는 점이다.',
    output: '사실: 독립 external runner이며 deep-interview, ralplan, ultragoal, 선택적 team과 네 기본 역할을 앞세운다. 장점: 사용자에게 보이는 기본 방법이 비교적 작고 MIT다. 단점: 내부에는 TypeScript·Rust·Python, 터미널 화면, tmux, RPC, Telegram, notebook research가 있어 배포물은 가볍지 않으며 스스로 experimental beta라고 밝힌다.',
  },
  {
  ...
  },
  {
    ...
    output: '기본 정책: 읽기·검색은 economy/low, 분석·계획은 frontier/max, 구현은 balanced/medium, 사실 검토는 frontier/high, 쉬운 설명은 balanced/low, 마지막 독자 검수는 economy/low 10회다. complexity, risk, blastRadius, verifiability, reversibility를 0~3으로 기록한다. 이 점수는 실행 전 초기 라우팅이며 0.1은 실패 후 자동 승격하지 않는다. Codex task-level /goal로 이번 출시 목표를 추적했지만 Relay10 자체에는 /goal DSL이 없다.',
  },
  {
...
const nextSteps = [
  ...
  '최종 10회 실제 독자 검수는 npm run audit:launch로 실행한다. 9/10 미만 또는 치명적 문제 1개 이상이면 출시하지 않는다.',
  '실패하면 outputs/relay10-launch-reader-live.json의 “막힌 모호점” 목록을 고치고 HTML을 다시 생성한 뒤 10회 전체를 재실행한다.',
  '검수 통과 후 gh repo view minwoo19930301/relay10, gh release view v0.1.0 --repo minwoo19930301/relay10로 공개 상태를 확인한다.',
