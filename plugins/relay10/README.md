...
The pack follows the Agent Skills layout with two plugin manifests: `.codex-plugin/plugin.json` for Codex and `.claude-plugin/plugin.json` for Claude Code. It can guide work in either host and call an installed `r10` CLI, but it does not switch the model of the host's current task, add a provider, or provide a native app UI or MCP server. Relay10 model stages still launch Codex CLI subprocesses.
...
A cloned repository also exposes the same skills directly: `.agents/skills` and `.claude/skills` are relative symlinks to the canonical pack at `skills/`. The per-skill `agents/openai.yaml` files are Codex interface metadata; Claude Code ignores them and loads each skill from its `SKILL.md`.
