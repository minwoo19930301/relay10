...
function renderHarnessComparison(comparison) {
  ...
  return `<div class="table-wrap" tabindex="0" role="region" aria-label="하네스 장단점과 Boundrun 체리피킹 표">
    <table class="wide-table">
      <thead><tr><th scope="col">하네스</th><th scope="col">장점</th><th scope="col">단점</th><th scope="col">Boundrun이 채택한 패턴</th><th scope="col">의도적으로 제외한 부분</th></tr></thead>
      <tbody>${rows.map((row, index) => {
...
function renderGlobalRepos(repositories) {
  ...
  return `<div class="table-wrap" tabindex="0" role="region" aria-label="글로벌 상위 저장소 증류 표">
    <table class="wide-table">
      <thead><tr><th scope="col">프로젝트</th><th scope="col">현재 신호</th><th scope="col">강점</th><th scope="col">약점·주의</th><th scope="col">Boundrun 채택</th><th scope="col">Boundrun 제외</th></tr></thead>
      <tbody>${rows.map((row, index) => {
        const item = row && typeof row === 'object' ? row : { name: row };
...
function renderSkillPack(skills) {
  ...
  return `<div class="table-wrap" tabindex="0" role="region" aria-label="Boundrun Skill pack 표">
    <table class="support-table">
...
export function generateReport(data = {}, options = {}) {
  const title = data.title ?? options.title ?? 'Boundrun 실행 보고서';
...
<body>
  ...
  <header class="hero">
    <div class="shell">
      <p class="eyebrow">Boundrun · 실행 보고서</p>
  ...
  <main id="main" class="shell">
    ...
    ${hasSkillPack ? `<section id="skills" aria-labelledby="skills-title"><p class="eyebrow">Skill · progressive disclosure</p><h2 id="skills-title">여덟 개만 남긴 Boundrun Skill pack</h2>${renderText(data.skillSummary ?? 'Skill은 한 가지 일을 맡고 필요할 때만 로드됩니다. Plugin은 이 Skill들을 배포하는 묶음이며 현재 task 모델을 바꾸지 않습니다. 스킬 id는 당분간 relay10-* 레거시 이름을 유지합니다.')} ${renderSkillPack(skillPack)}</section>` : ''}
    ${hasComparisons ? `<section id="comparison" aria-labelledby="comparison-title"><p class="eyebrow">비교 · 설계 계보</p><h2 id="comparison-title">하네스별 장단점과 Boundrun 체리피킹</h2>${renderText(data.comparisonSummary ?? 'Boundrun(구 Relay10)은 비교 대상의 코드를 복사하지 않고, 검증 가능한 동작 패턴만 선택적으로 독립 구현했습니다.')} ${renderHarnessComparison(comparisons)}</section>` : ''}
  ...
  <footer class="shell"><p>이 파일은 외부 스크립트나 스타일시트 없이 생성된 자체 포함 Boundrun 보고서입니다.</p></footer>
</body>
</html>`;
}
