# Relay10 30/60/90 development and promotion playbook
...
외부 사용자 수**다. Relay10 core에 추적 telemetry를 넣지 않는다. 첫 성공 날짜,
...
- macOS와 Linux에서 `clone → npm link → r10 doctor → route → dry-run`을 한
...
> Show GN: Codex를 바꾸지 않고 단계별 모델 역할과 근거를 남기는 Relay10
...
> Show HN: Relay10 – a small evidence-first router for Codex tasks
