...
- Grok은 Relay10의 직접 지원 공급자가 아니다. Codex의 custom model
...
| Codex + xAI/Grok custom provider | **실험 후보·미검증** | Codex custom provider와 xAI가 모두 Responses 표면을 제공한다. 그러나 Relay10은 provider를 선택하거나 capability를 검사하지 않는다. | user-level Codex xAI profile, 모델 override, effort 정규화, file tool·search·schema·Reader-10 E2E 검증. |
