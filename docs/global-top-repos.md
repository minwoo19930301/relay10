# Global top repositories: a DisciplinedRun distillation
...
or compatibility with DisciplinedRun. A recent `pushed_at` timestamp is also only an
...
| Repository | Stars | Last push | Repository license / API signal | Strongest idea | Cost or limitation | DisciplinedRun decision |
|---|---:|---|---|---|---|---|
...
default DisciplinedRun path. [Continue](https://github.com/continuedev/continue)
...
a useful worktree/session comparison. DisciplinedRun does not inherit benchmark claims,
...
| Repository | Stars | Last push | License | Strongest skill pattern | Cost or limitation | DisciplinedRun decision |
|---|---:|---|---|---|---|---|
...
| [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) | 77,810 | 2026-07-12 | MIT | Lifecycle skills use vertical slices, test evidence, debugging gates, review, and progressive disclosure. | A 24-skill pack plus extensive rule tables can consume context and become a second process framework. | **Take:** evidence gates, safe slices, and stop/reassess behavior in build and debug skills. **Leave:** bulk loading and generic rules unrelated to DisciplinedRun. |
...
## The distilled DisciplinedRun skill pack

DisciplinedRun therefore keeps eight clean-room skills, each with one primary job.
The names and instructions are local DisciplinedRun work; the table records abstract
influences, not copied text or code.

| DisciplinedRun skill | Job | Patterns retained | Deliberate boundary |
|---|---|---|---|
...
DisciplinedRun's differentiator remains the combination already present in its own
...
3. An MIT or Apache-2.0 repository still carries attribution and notice duties
   if code is copied. DisciplinedRun avoids that branch of work by implementing only
